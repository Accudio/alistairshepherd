...
do
  ...
  # webm 480
  ffmpeg -i $i -y -c:v libvpx-vp9 -pass 1 -b:v 0 -crf 53 -threads 8 -speed 4 -vf scale=640:-2 -tile-columns 6 -frame-parallel 1 -an -f webm /dev/null && \
  ffmpeg -i $i -y -c:v libvpx-vp9 -pass 2 -b:v 0 -crf 53 -threads 8 -speed 2 -vf scale=640:-2 -tile-columns 6 -frame-parallel 1 -auto-alt-ref 1 -lag-in-frames 25 -an -f webm $of-480.webm
  ...
  # mp4 480
  ffmpeg -i $i -y -c:v libx264 -vf scale=640:-2 -preset veryslow -pix_fmt yuv420p -profile:v baseline -level 3.0 -movflags +faststart -crf 33 -threads 8 -speed 0 -an $of-480.mp4
